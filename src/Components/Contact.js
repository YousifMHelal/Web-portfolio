...
const Contact = ({ theme }) => {
  ...
  const sendEmail = (e) => {
    ...
    if (fName_input.value !== '' && lName_input.value !== ''
      && email_input.value !== '' && message_input.value !== '')
    {
      emailjs.sendForm('service_pk4asxc', 'template_7xrzrda', e.target, 'c_cerBrsyLHJ-Hpbw')
        .then(() => {
          ...
          theme === 'dark'
            ? <Fade top>
              <form ref={form} onSubmit={sendEmail} className='flex flex-col items-center'>
                ...
                <button
                  type="submit"
                  className="btn dark-blue-btn mt-4">
                  Send your massage</button>
              </form>
            </Fade>
